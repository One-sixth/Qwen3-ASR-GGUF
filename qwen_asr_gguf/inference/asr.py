...
class QwenASREngine:
    ...
    def __init__(self, config: ASREngineConfig, decode_other_kwargs: dict = None):

        # 额外的解码配置
        if decode_other_kwargs is None:
            decode_other_kwargs = {}
        self.decode_other_kwargs = decode_other_kwargs
    ...
    def _decode(
        ...
        streaming: bool = True,
        *,
        top_k: int = 50,                # 仅保留概率最高的 top_k 个 token，其余 token 概率置零，再重新归一化。
        top_p: float = 1.0,             # 又称 Nucleus Sampling。从概率最高的 token 开始累加，直到累计概率超过 top_p，然后仅在此“核心”集合中采样。
        min_p: float = 0.0,             # 相对阈值过滤。移除所有概率低于 max_prob * min_p 的 token（max_prob 是当前最高概率 token 的概率）。
        repeat_penalty: float = 1.0,    # 若 token 已出现在最近的 penalty_last_n 序列中，其 logit 除以 repeat_penalty。
        frequency_penalty: float = 0.0, # 出现次数越多的 token，被重复选中的概率越低，有助于抑制高频率词的机械重复。
        presence_penalty: float = 0.0,  # 基于 token 是否在序列中出现过（至少一次）施加惩罚，frequency_penalty 会随出现次数线性增加惩罚，而 presence_penalty 是固定的二进制惩罚。
        penalty_last_n: int = 64,       # 只考虑最近生成的 penalty_last_n 个 token 来计算惩罚。
        ...
        sampler = llama.LlamaSampler(
            ...
            repeat_penalty=repeat_penalty, frequency_penalty=frequency_penalty, presence_penalty=presence_penalty,
    ...
    def _safe_decode(
        ...
        """带熔断加温重试的高层推理封装"""
        for i in range(4):
            res = self._decode(full_embd, prefix_text, rollback_num, is_last_chunk, temperature, streaming=streaming, **self.decode_other_kwargs)
